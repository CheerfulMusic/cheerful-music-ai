...
function importRowRecords(batchId,platform,rows,mapping,defaultCurrency,defaultPeriod){return rows.map((row,rowIndex)=>({
 batchId,rowIndex,platform,rawData:row,
 title:mapping.title?row[mapping.title]:'',artist:mapping.artist?row[mapping.artist]:'',isrc:mapping.isrc?row[mapping.isrc]:'',
 country:mapping.country?row[mapping.country]:'',quantity:mapping.quantity?numberValue(row[mapping.quantity]):0,
 revenue:mapping.revenue?numberValue(row[mapping.revenue]):0,currency:mapping.currency?row[mapping.currency]:defaultCurrency,
 period:mapping.period?row[mapping.period]:defaultPeriod
}))}
...
window.renderRoyaltyImportManager=function(){
 ...
 <div class="finance-note">导入批次、完整收入明细和解析摘要均保存到 Supabase；刷新或更换电脑后仍可继续匹配、计算和审核。</div>`
}
